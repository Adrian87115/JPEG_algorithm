...
void ProcessedImage::printHuffmanTree(HuffmanNode* root, string code){
    if(!root){
        return;
    }
    if(isLeaf(root)){
        cout << "Value: " << root->value << ", Frequency: " << root->frequency << ", Code: " << code << endl;
    }
    printHuffmanTree(root->left, code + "0");
    printHuffmanTree(root->right, code + "1");
}

void ProcessedImage::callPrintHuffmanTree(vector<vector<int>> rle_data){
    for(int ch = 0; ch < rle_data.size(); ch++){
        map<int, int> freq_map;
        for(int val : rle_data[ch]){
            freq_map[val]++;
        ...
        HuffmanNode* root = buildHuffmanTree(values, frequencies);
        tree_channels[ch] = root;
        cout << "Channel " << ch << " Huffman Tree:" << endl;
...
vector<vector<string>> ProcessedImage::readFromFileEncoded(){
    vector<vector<string>> encodedData;
    ifstream infile(img_name + ".txt");
    if(!infile.is_open()){
        cerr << "Error: Unable to open the file" << endl;
        return encodedData;
    }
...
vector<map<string, int>> ProcessedImage::readFromFileMap(){
    vector<map<string, int>> codes;
    ifstream infile(img_name + "_codes.txt");
    if(!infile.is_open()){
        cerr << "Error: Unable to open the file" << endl;
        return codes;
    }
...
QImage* ProcessedImage::decode(vector<vector<string>> encoded_data){
    vector<vector<int>> decoded_huffman = HuffmanDecoding(encoded_data);
    // cout << "Huffman decoded\n";
    ...
    // cout << "Blocks reversed\n";
    QImage upscaled = upscaling(blocks_reversed);
    // cout << "Data upscaled\n";
    QImage color_converted = YCbCrToRGB(upscaled);
    // cout << "RGB converted\n";
    QImage* decoded_image = new QImage(color_converted);
    // cout << "Successfully decoded\n";
    return decoded_image;
}

QImage* ProcessedImage::decodeFromFile(){
    vector<vector<string>> readed_data = readFromFileEncoded();
    QImage* decoded_image = decode(readed_data);
    return decoded_image;
}

QImage* ProcessedImage::decodeFromFiles(){
    vector<vector<string>> readed_data = readFromFileEncoded();
    // cout << "Encoded data readed\n";
    vector<map<string, int>> codes = readFromFileMap();
    // cout << "Maps readed\n";
...
vector<vector<vector<vector<int>>>> castToInteger(vector<vector<vector<vector<float>>>> data){
    vector<vector<vector<vector<int>>>> int_data(3);

    for(int ch = 0; ch < 3; ch++){
        int_data[ch].resize(data[ch].size());
        for(int block_y = 0; block_y < data[ch].size(); block_y++){
            int_data[ch][block_y].resize(data[ch][block_y].size());
            for(int block_x = 0; block_x < data[ch][block_y].size(); block_x++){
                int_data[ch][block_y][block_x].resize(data[ch][block_y][block_x].size());
                for(int i = 0; i < data[ch][block_y][block_x].size(); i++){
                    int_data[ch][block_y][block_x][i] = static_cast<int>(round(data[ch][block_y][block_x][i]));
                }
            }
        }
    }
